...
func main() {
	gocache = cache.New(1*time.Second, 10*time.Minute)
...
func makeLeaderboardPB(e echo.Context, teamID int64) (*resourcespb.Leaderboard, error) {
	...
	cached, found := gocache.Get("leaderboard")
	if found {
		var ok bool
		pb, ok = cached.(*resourcespb.Leaderboard)
		if !ok {
		...
		gocache.Set("leaderboard", pb, cache.DefaultExpiration)
	}
	return pb, nil
}
